...
### 2.3.1. Три слоя (не смешивать)

| Слой | Кто решает | Правило |
|------|------------|---------|
| **Посадка игроком** | Игрок | Семена можно поставить на любую подходящую **физически** клетку (твёрдая почва, пусто сверху, claims). Без проверки климата. |
| **Выживание особи** | Игра + мод | После посадки климат/почва определяют, **дорастёт ли** `wildplant` до взрослого или **погибнет** (повторные проверки, затем `BreakBlock`). |
| **Дикое размножение** | Экосистема | Только у **зарегистрированных** взрослых растений (`IReproducible` / реестр в `EcosystemSystem`). Попытка потомства — только клетки с `Score >= MinFitness`; иначе ничего не ставить. |

«Как отслеживать в дикой природе» — не сканировать весь мир: учитываются только взрослые блоки, которые мод **сам зарегистрировал** при созревании (или позже — при появлении ванильного цветка с behavior). Остальной мир — ванильный worldgen; игрокские эксперименты — слой выживания, не реестр.
