...
See [`TREE_AGING.md`](TREE_AGING.md). Species lifespans are **hardcoded** (`WildTreeGrowthProfiles.cs`), not config keys.
...
See [`CANOPY_PHENOLOGY.md`](CANOPY_PHENOLOGY.md).
...
See [`CANOPY_AMBIENCE.md`](CANOPY_AMBIENCE.md).
...
See [`PHASE6_SIMULATION.md`](PHASE6_SIMULATION.md).
...
Block JSON (not in `ecosystemflora.json`): `ecologySpreadMode` — `rhizome`, `surfacemat`, or `independent`. See [`THIRD_PARTY_ECOLOGY.md`](THIRD_PARTY_ECOLOGY.md).
...
For release history see [`CHANGELOG.md`](CHANGELOG.md). Keys added since **3.1.2** are listed there and in older ModDB paste blocks; this file is the **complete** current set.
