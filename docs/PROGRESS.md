...
- [ ] **Understory без symbiosis** — хвощ: playtest после срубки (niche stress, не только symbiosis)
...
- [ ] **Сукцессия почвы (мягкая модель)** — таблица per-species: при **успешной посадке** и при **смерти** (stress / symbiosis / displacement) менять блок почвы под клеткой и/или состав (`SoilKind`, fertility, торф/forestfloor и т.д.); хвощ увлажняет, луговые «высушивают», лесная understory — forestfloor. Без жёсткого запрета spread вне ниши — почва как обратная связь, не gate.
