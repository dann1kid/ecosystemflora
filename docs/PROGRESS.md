...
| Слой | Поведение |
|------|-----------|
| **Объект** | Ванильные блоки; регистрация — `RegistrationScanQueue` (priority + background) + burst; **Phase A** `PendingRegistrationQueue` (paced apply); **background scan** — snapshot on main, classify on worker (`BackgroundRegistrationScanner`); сезонная крона — `FoliageChunkSyncPass` на main при фоновом скане; **деревья** — `log-grown` / ferntree в column pass |
