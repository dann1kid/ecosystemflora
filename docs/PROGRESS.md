...
**Текущая стадия:** `Ecosystem v3.6.0` — wild tree maturation + calendar age persistence (см. [`TREE_AGING.md`](TREE_AGING.md)); prior **3.5.0** canopy ambience, **3.2.0** seasonal canopy phenology; **3.1.12** mycelium + tree spread.  
**Версия мода:** `3.6.0` · **Игра:** Vintage Story 1.22+ · **Сборка:** .NET 10 · **Тесты:** xUnit (**253**)  

**ModDB:** https://mods.vintagestory.at/ecosystemflora  

Последнее обновление документации: 2026-06-14 (код **3.6.0**, tree maturation + senescence + save persistence).
...
| **Деревья** | `log-grown-{wood}-*` → spread `sapling-{wood}-free` (14 пород); **v3.6** — годовое укрупнение, senescence death, persist возраста — [`TREE_AGING.md`](TREE_AGING.md) |
...
| `EnableTreeAging` / `EnableTreeSenescence` | **v3.6:** годовой рост grown-блоков; senescence = полное снятие дерева; возраст в savegame moddata — [`TREE_AGING.md`](TREE_AGING.md) |
...
| **3.6.0** | **Wild tree maturation** — calendar age + structure growth; senescence = full tree removal; age persists in savegame; inspect (I) on trunk; [`TREE_AGING.md`](TREE_AGING.md) |
...
| 5 | **Unit-тесты** — чистые функции (scoring, classification, season, mat spread, hand harvest, mycelium, canopy, tree growth/senescence, config merge) | [x] 253 tests |
