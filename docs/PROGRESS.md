...
- [x] **Сукцессия почвы (tier-only)** — `WildSoilComposition` (tier + `TierProgress`, влажность RAM), роли incl. `NitrogenFixer` (lupine), spread/death → `soil-*`/`forestfloor`/`peat`; spread на farmland запрещён
- [x] **Мост на пашню** — `FarmlandTillBridge` после вспашки (`DidUseBlock` + тик 50 ms): N/P/K по доминирующей роли + tier; конфиг `UseFarmlandNutrientBridge`
- [ ] **Сукцессия почвы — persist** — chunk moddata для agro/tier progress
- [ ] **Залежь** — farmland без культуры + дикая трава → медленный N в BE
