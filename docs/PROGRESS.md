...
| Слой | Поведение |
|------|-----------|
| **Объект** | Ванильные блоки; цветы/водные — патч `EcoSystemLife`; **деревья** — скан чанка по `log-grown` (без патча на ствол) |
...
| Файл | `entityClass` |
|------|----------------|
| `flower.json` | `EcoSystemLife` |
| `flower-lupine.json` | `EcoSystemLife` |
| `tallgrass.json` | `EcoSystemLife` |
| `reedpapyrus.json` | `EcoSystemLife` |
| `waterlily.json` | `EcoSystemLife` |
| `aquatic/watercrowfoot.json` | `EcoSystemLife` |
...
- [ ] **Chunk-scan без `EcoSystemLife` BE** — техдолг: регистрация только из `ChunkFlowerScanner`, убрать `entityClass` на блоках
