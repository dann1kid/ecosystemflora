...
|-----|------------|--------|
| 1 | **Сезонность** — множители spread/stress по месяцу/сезону, зимняя выживаемость per species | [ ] |
| 2 | **Strip** legacy JakeCool (мёртвый код/assets вне сборки) | [ ] |
| 3 | **Rename** `EcosystemPlant` → `EcoSystemLife` | [ ] |
| 4 | Рефактор + **perf-анализ** (фаза 3 — по результатам) | [ ] |
| 5 | ModDB + hotfix по отзывам | [ ] |
...
### v2.3 — сезонность (черновик)

- [ ] `WildSpeciesSeason` — пик spread в начале сезона, `OffSeasonSpreadFactor`, `WinterHardiness`
- [ ] Множитель на `SpeciesSpread` / `ReproduceChance` от `Calendar` (месяц или сезон)
- [ ] Конец сезона — усиленное отмирание (stress/death) у «однолетних»; часть трав переживает зиму
- [ ] (позже) кривая 12 месяцев per species, если MVP-таблицы мало
...
- [ ] **Chunk-scan без `EcosystemPlant` BE** — техдолг: регистрация только из `ChunkFlowerScanner`, убрать `entityClass` на блоках (после rename → `EcoSystemLife` или вместо него)
...
#### Фаза 3 — только при лагах на большом реестре

**Триггер:** профилирование при ~18k+ reproducers или жалобы на TPS; до ModDB не блокер.
