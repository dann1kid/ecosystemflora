...
**Текущая стадия:** `Ecosystem v3.1.9` — spread/debris + Terrain Slabs succession fix; плюс legacy BE 3.1.8, meadow harvest 3.1.7, aquatic mat 3.1.3–6, soil succession 3.1.2.  
**Версия мода:** `3.1.9` · **Игра:** Vintage Story 1.22+ · **Сборка:** .NET 10 · **Тесты:** 112 (xUnit)  
**ModDB:** https://mods.vintagestory.at/ecosystemflora  

Последнее обновление документации: 2026-05-29 (код под modinfo **3.1.9**).
...
## Текущая модель (Ecosystem v3.1.9)

Дикая экосистема на **ванильных** блоках родителей (`game:`). Сторонние blocktypes — **`ecologyParticipant`** ([`THIRD_PARTY_ECOLOGY.md`](THIRD_PARTY_ECOLOGY.md)). Валидация баланса: **логи** + **осмотр (I)** — mat edge, seed %, stress, season.
...
**Осмотр (I):** spread mode, mat edge yes/no, seed % — см. lang `inspect-line-spread-mode-*`, `inspect-line-mat-frontier-*`.
...
| Осмотр (I) краш GUI | `EcologyInspectDialog` — фиксированный размер диалога вместо `FitToChildren` |
...
| 5 | **Unit-тесты** — чистые функции (scoring, classification, season, mat spread, hand harvest) | [x] 112 tests |
