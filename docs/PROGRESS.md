...
**Текущая стадия:** `Ecosystem v4.11.25` — per-world config + setup wizard; potato/X3D perf; tree niche lifespan stress + lean niche sample; crown forms; Floral Zones 7/7; third-party wild ecology.  
**Версия мода (ModDB / код):** `4.11.25` · **Игра:** Vintage Story 1.22+ · **Сборка:** .NET 10 · **Тесты:** xUnit (**882+**)  

**ModDB:** https://mods.vintagestory.at/ecosystemflora  

Последнее обновление документации: 2026-07-22 (код **4.11.25**, [`CHANGELOG.md`](CHANGELOG.md)).
...
## Текущая модель (Ecosystem v4.11 / flower phenology v3.9.24)
...
**Конкуренция за клетку:** spread + displacement + stress + symbiosis. Aquatic (reeds, lily, **crowfoot**) — **mat spread** (edge + rare seed где настроено), не independent radius-search.
...
| Группа | Блоки |
|--------|--------|
| **Цветы** | `game:flower-*` (23 вида + colonizers) | Spread → juvenile → mature; **фенология** dormant/vegetative/bloom/dieback (v3.9.24 — [`FLOWER_PHENOLOGY.md`](FLOWER_PHENOLOGY.md)); maturation — [`FLOWER_SPREAD_MATURATION.md`](FLOWER_SPREAD_MATURATION.md) |
| **Луг** | `tallgrass-*` — матрица под цветами; spread → **veryshort**, vanilla рост до **short+** (v3.9.7 — [`TALLGRASS_SPREAD_MATURATION.md`](TALLGRASS_SPREAD_MATURATION.md)) |
...
| **Скорость** | `spread_rate` из **`SpeciesEcologyRegistry`** (CSV); × `SpeciesSpreadRateScale` в JSON; календарь (`ReproduceAttemptsPerYear`) или legacy часы |
| **Spacing** | `same_species_spacing` / `other_species_spacing` / `spacing_from_species` в CSV; конфиг `Default*Spacing`; у тростника нет вертикального стека в колонке |
...
1. **Семя** — зрелый `log-grown` ставит **young log-grown seedling** (не vanilla sapling; зима выкл., не на лёд/снег) — **4.7**.
2. **Рост** — yearly maturation через `TreeYoungStructure`; при полном размере — обычный aging loop; player-placed saplings — ванильный treegen.
...
- **Потомство:** young `log-grown` seedling (wild spread); player sapling — vanilla treegen.
...
**Spread:** **RhizomeMat** edge ±1 (`CrowfootMatSpread`, `MatSpreadDispatch`); отдельная underwater-колонка, не reed-топология. `ecology.csv`: spread **0.75**, глубина **2–6**, seed **0**. См. [GAPS.md](GAPS.md) §2.

- Spread ставит колонку от `aquatic-watercrowfoot-section`, сверху `tip` или `top` (~35% с цветами).
- Guards: fish trap / solid / BE (`CrowfootSpreadGuard`); снег не красит submerged cells (`PlantSnowCover`).
...
| `watercrowfoot` | водяной лютик | rhizome mat-edge; подводная колонка 2–6 |
...
| **3.9.24** | **Flower phenology** — simulated phases, bloom-gated spread/harvest, 72 phase blocktypes, inspect + config; [`FLOWER_PHENOLOGY.md`](FLOWER_PHENOLOGY.md) |
| **3.9.23** | Juvenile flower texture fix — vanilla wildcard paths for catmint, heather, etc. |
| **3.9.20** | Flower spread maturation — all meadow flowers; spread cooldown polish; [`FLOWER_SPREAD_MATURATION.md`](FLOWER_SPREAD_MATURATION.md) |
| **3.9.8** | **Handbook server fix** — `RegisterCollectibleBehaviorClass` for `ecosystemHandbook` (dedicated server crash) |
| **3.9.7** | **Tallgrass spread maturation** — spread veryshort; reproduce from short+; promotion queue; [`TALLGRASS_SPREAD_MATURATION.md`](TALLGRASS_SPREAD_MATURATION.md) |
| **4.4.1** | **`SpeciesSpreadRateScale`** (~⅓ default); shore sedge retune; meadow scythe harvest fix |
| **4.5.0** | **Species CSV registry** — runtime `ecology.csv` + `season.csv`; ModConfig overrides — [`SPECIES_ECOLOGY_CSV.md`](SPECIES_ECOLOGY_CSV.md) |
| **4.5.1** | Brownsedge knife crash; harvested root break visuals |
| **4.5.2** | Fern rhizome placement crash; fern phase textures; spread null guards |
| **4.5.3** | Wildgrass / Wildgrass Fork patch fixes (`dependsOn`, `addmerge`) |
| **4.5.4** | CSV parity tests; `/ecospeciesreload`; [`CONFIGURATION.md`](CONFIGURATION.md) (209 keys) |
| **4.7.0** | **Third-party wild ecology** — Wildcraft Fruit/Trees, Floral Zones (5 regions), fruitvine climate-only, B+ auto-curves; **crowfoot mat-edge**; wild tree log-grown seedlings; seasonal snow; wildfire canopy; wild vine hang/corner; `ecosystemfloracompat` submodule — [`CHANGELOG.md`](CHANGELOG.md) |
| **4.9.13** | Floral Zones **Cape + Cosmopolitan** (211 entries, **7/7** regions); hitch/trails/phenology senescence polish — [`CHANGELOG.md`](CHANGELOG.md) |
| **4.10.x** | Crown forms; vine host revalidation; lighter main defaults; **per-world config** (`ModConfig/ecosystemflora/worlds/…`) |
| **4.11.15–20** | Setup wizard + `/ecosetup` / `/ecoautotune`; Potato / X3D / Super-minimal perf profiles; dedicated-server setup UX |
| **4.11.22–25** | **Tree niche lifespan stress** (debt → earlier senescence); immature spread gate; worldgen-temp + own-crown-excluded forest sample; lean niche sample perf — [`TREE_AGING.md`](TREE_AGING.md) |
| **5.0 (planned)** | **Phase 7** — external ecology sim for **unloaded** chunk columns (DB + optional Go worker, catch-up on load) — [`PHASE7_EXTERNAL_SIMULATION.md`](PHASE7_EXTERNAL_SIMULATION.md) |
...
| **v4.7–4.11 third-party** | Wildcraft / Floral Zones 7/7 runtime bootstrap; fruitvine climate-only; playtest в мире с модами | [x] код; [ ] playtest |
| **v4.11 polish** | de handbook; dominant UX; Floral Zones trees; crowfoot worker frontier gate | [ ] [GAPS.md](GAPS.md) |
| **v5.0 Phase 7** | Unloaded chunk ecology — snapshot export, ecology DB, optional `ecology-sim` process, paced apply on load | [ ] design [`PHASE7_EXTERNAL_SIMULATION.md`](PHASE7_EXTERNAL_SIMULATION.md) |
...
| 5 | **Unit-тесты** — чистые функции (scoring, classification, season, mat spread, hand harvest, mycelium, canopy, tree growth/senescence, ferntree, vines, config merge, Phase 6 schedulers, spread maturation) | [x] 395 tests |
...
| **Trampling** — тропы от шагов (`EnableTrampling`, default on): column pressure, plant wear, soil wear, fitness penalty | [x] v2.6 → ecological trails |
...
### v2.6 — протаптывание дорожек (trampling → foot traffic)

Растения и почва под ногами: колонка `XZ` копит `ColumnTraffic` pressure (savegame); tallgrass теряет ступень высоты; остальные terrestrial снимаются после N шагов; покрытие почвы синхронизируется с pressure (`normal`↔`verysparse`, restore при спаде; не до голого `none`); age/prune на world save; tempo — `FootTrafficStepsToFullCoverageWear`; spread/hold × `TrafficMultiplier`. Не path-мод.

- [x] `ColumnTrafficStore` + lazy decay
- [x] `FootTrafficService` — `EntityPlayer.OnFootStep` (игроки); животные — `EntityBehaviorFootTraffic` + `PhysicsUpdateWatcher` stride (`EnableAnimalFootTraffic`)
- [x] Конфиг: `EnableTrampling`, `FootTraffic*`, `TramplingRadius`, `TramplingStressThreshold`, `TramplingSoilDegradation`
- [x] Fitness/hold penalty + `SpreadSolveCell.TrafficFitnessMult`
- [x] Inspect (I): traffic + plant wear
- [ ] Playtest: заметная тропа при повторном хождении по лугу
...
- [x] `ecologyParticipant` + attrs на blocktypes; `EcosystemParticipant.TryFromBlock`
- [x] Habitat templates (`ReedNearWater`, `WaterSurface`, …)
- [x] Wildgrass optional JSON patches (`wildgrass-ecology.json`, handbook)
- [x] **v4.7** — runtime bootstraps: `WildcraftFruit*`, `WildcraftTree*`, `FloralZones*` (worldgen climate table); fruitvine `ecologySpreadRate: 0`; `DiscoveredSpeciesStore` + `DynamicSpeciesAutoCurves` (B+)
- [x] **ecosystemfloracompat** — submodule `community/` (Biodiversity bdshrub/bdherb); см. [`SUBMODULES.md`](SUBMODULES.md)
- [ ] Playtest Wildcraft + все 7 Floral Zones в одном мире
...
- [ ] Playtest aquatic tempo: crowfoot mat vs reeds/lily в одном озере; камыш в жаре, лютик max depth
...
| `TickBudgetMs` | Потолок ms на **prep** reproduce (maturation/phenology/trees; default **5**); 0 = без лимита; не съедает spread |
| `SpreadBudgetMs` | Отдельный потолок spread (default **4**; 0 = `TickBudgetMs`) |
...
| `ChunkScanTickIntervalMs` | Интервал registration scan (default **1000** ms; clamp &lt;100 unless preset `timelapse`) |
...
#### Фаза 7 — external simulation (unloaded chunks) — planned v5.0

План: [`PHASE7_EXTERNAL_SIMULATION.md`](PHASE7_EXTERNAL_SIMULATION.md). Цель — экология **выгруженных** chunk columns; VS применяет накопленные diffs на main thread.

- [ ] **7.1 Ecology snapshot** — compact column export on unload (не полный region file)
- [ ] **7.2 Ecology DB** — per-world SQLite; reproducers + `pending_ops` + `lastSimGameHour`
- [ ] **7.3 Catch-up on load** — paced `SetBlock` + registry merge (`ExternalSimCatchUpBudgetMs`)
- [ ] **7.4 Process host** — `EcologySimProcessHost`: spawn/kill Go worker; PID stale recovery; server-only
- [ ] **7.5 Go worker MVP** — batch fast-forward; localhost IPC; graceful shutdown
- [ ] **7.6 Rule parity** — terrestrial + mat spread/phenology; species CSV shared with C#
- [ ] **7.7 Tiered sim** — hot/warm/cold columns; config + handbook

Phase 6 (loaded chunks) остаётся in-process; Phase 7 **дополняет**, не заменяет на первом этапе.

### Завершено (контент)

- [x] Деревья, ягоды, папоротники, почва (`SoilKind`), aquatic (код)
- [x] **Species CSV registry** — `SpeciesEcologyRegistry` + `SpeciesSeasonRegistry`; export-only C# `Wild*Ecology`; inspect/handbook via `SpeciesEcologyDisplay` — [`SPECIES_ECOLOGY_CSV.md`](SPECIES_ECOLOGY_CSV.md)
...
- Конфиг JSON: `%AppData%\VintagestoryData\ModConfig\ecosystemflora.json`
- Конфиг CSV: `%AppData%\VintagestoryData\ModConfig\ecosystemflora\species\ecology.csv`, `season.csv` — [`SPECIES_ECOLOGY_CSV.md`](SPECIES_ECOLOGY_CSV.md)
