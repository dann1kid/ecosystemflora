...
**Сейчас:** `SoilKind` + fertility min/max (`WildPlantSoil`), `MinSunlight` у папоротников/ягод/травы, `FloraContext` (open/edge/forest) как прокси тени. **Нет** явных уровней влажности почвы и градаций освещённости для всех видов.
...
- [ ] **Модель уровней** — enum или шкала `MoistureLevel` (dry / mesic / wet / submerged margin) и `LightLevel` (open / partial / shade / deep shade); маппинг из блока почвы + соседней воды + `TreePlacement`/соседей-кроны
- [ ] **Сэмплер локальной ниши** — `NicheSampler` или расширение `EnvironmentalContext`: влажность (торф, muddygravel, rain, fluid nearby), свет (sunlight 0–24 или категории под кроной)
- [ ] **Таблица видов** — `WildSpeciesNiche` / поля в `PlantRequirements`: `PreferredSoils`, `MinMoisture`/`MaxMoisture`, `MinLight`/`MaxLight` (или affinity + bonus как у `FloraContext`)
- [ ] **Fitness / stress** — множитель spread и stress death при выходе из ниши (хвощ → wet+shade; полевые → dry+open; ландыш → mesic+shade)
- [ ] **Understory без symbiosis** — хвощ и др. «лесные влаголюбы» уходят после срубки через падение shade/moisture, не только `FloraSymbiosis`
- [ ] **Aquatic vs берег** — влажность/субстрат для водных: ил vs сухой берег; spacing не блокирует spread с непригодного берега (см. playtest)
- [ ] Playtest: лесная прогалина, берег озера, открытый луг — разные домinant species
