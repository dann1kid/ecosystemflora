...
**Текущая стадия:** `Ecosystem v3.1.12` — mycelium niche + ecology + inspect (I); плюс tree spread 3.1.11, meadow harvest 3.1.10.  
**Версия мода:** `3.1.12` · **Игра:** Vintage Story 1.22+ · **Сборка:** .NET 10 · **Тесты:** 158 (xUnit)  

**ModDB:** https://mods.vintagestory.at/ecosystemflora  

Последнее обновление документации: 2026-06-13 (код под modinfo **3.1.12**).

См. также: [PROJECT_VISION.md](PROJECT_VISION.md) (теория), [PROMPT.md](PROMPT.md) (промпт для агентов), [THIRD_PARTY_ECOLOGY.md](THIRD_PARTY_ECOLOGY.md) (сторонние блоки), [GAPS.md](GAPS.md) (пробелы идеи).
...
## Текущая модель (Ecosystem v3.1.12)
...
| **Стресс (`EnableMyceliumEcology`)** | Луг в лесу / лес без tree-host → накопление failed checks → `RemoveBlockEntity`. Рубка дерева — каскад для лесных якорей (`MyceliumTreeCascade`). |
...
Конфиг: `EnableMyceliumNiche`, `EnableMyceliumEcology`, `EnableMyceliumNetworkSpread`, `MyceliumZoneRadius`, `MyceliumSpreadRate`, … — см. таблицу ниже.
...
- [x] `MyceliumEcology`, `MyceliumZone`, `MyceliumNetworkSpread`, `MyceliumInspect`, `MyceliumChunkRegistrar` — грибница v3.1.12
...
| `MyceliumSpreadRate` / `MyceliumSpreadAttemptsPerYear` | Темп network spread |
| `MyceliumTreeHostRadius` | Поиск tree-host для лесной грибницы |
...
| **3.1.12** | Mycelium niche + ecology + network spread; meadow coexistence; inspect (I) по шляпке и почве; `Mycelium*Tests` (+29) |
...
| 5 | **Unit-тесты** — чистые функции (scoring, classification, season, mat spread, hand harvest, mycelium) | [x] 158 tests |
