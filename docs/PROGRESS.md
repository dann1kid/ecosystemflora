...
Дикая экосистема на **ванильной** базе родителей (`game:`). При **`EnableThirdPartyParticipants: true`** к тем же правилам подключаются blocktypes других модов с **`ecologyParticipant`** (см. **[`THIRD_PARTY_ECOLOGY.md`](THIRD_PARTY_ECOLOGY.md)**). При снятии мода ванильные родители остаются ванильными.
...
Mod DB — **опубликовано** 2026-05-26. **`v3.0`** — traits ягод при spread; **`v3.1`** — контракт через JSON для сторонних модов ([`docs/THIRD_PARTY_ECOLOGY.md`](THIRD_PARTY_ECOLOGY.md)); следующий акцент — backlog (UX / aquatic edge cases) или v3.x polish.
...
| 5 | **Unit-тесты** — чистые функции (scoring, classification, season, inspect/survival) | [x] 72 tests |
...
- [ ] Playtest в игре: рост до mature/fruit с совпадающими traits родителя
