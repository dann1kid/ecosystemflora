...
**Версия мода:** `3.8.0` · **Игра:** Vintage Story 1.22+ · **Сборка:** .NET 10 · **Тесты:** xUnit (**323**)  

**ModDB:** https://mods.vintagestory.at/ecosystemflora  

Последнее обновление документации: 2026-06-14 (код **3.8.0**, Phase 6 complete, [`CHANGELOG.md`](CHANGELOG.md)).
...
| Слой | Поведение |
|------|-----------|
| **Объект** | Ванильные блоки; регистрация — очередь `PendingChunkScan` (priority + background) + burst near players; `ChunkFlowerScanner.ScanChunk` (возобновление с `(lx,lz)`); без `entityClass` патчей; **деревья** — скан чанка по `log-grown` |
...
| `WildVine` | `wildvine-end-*` tips | Downward extend + wall-face capture — [`WILD_VINE.md`](WILD_VINE.md) |
...
| 5 | **Unit-тесты** — чистые функции (scoring, classification, season, mat spread, hand harvest, mycelium, canopy, tree growth/senescence, ferntree, vines, config merge, Phase 6 schedulers) | [x] 323 tests |
...
- [x] **6.5 Two-phase placement** — `PendingSpreadQueue` evaluate + chunk-fair commit with revalidation
- [x] **6.6 Season coarse wake** — month-boundary wake for seasonal species; handbook + tuning docs
- [x] **6.7 Registration + spread collect** — `RegistrationScanQueue` priority/burst near players; `EnableEmptyFirstSpreadCollect` + `EcologyColumnOccupancy` hint (displacement when no vacancy)
...
| `EnableTwoPhaseSpreadPlacement` | Evaluate queue + chunk-fair SetBlock commit (default **true**) |
...
| `PlayerRegistrationPriorityRadiusBlocks` | Priority/burst radius (default **384**) |
