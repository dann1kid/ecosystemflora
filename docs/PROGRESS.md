...
**Текущая стадия:** `Ecosystem v3.6.0` — wild tree maturation + phased senescence + calendar age persistence (см. [`TREE_AGING.md`](TREE_AGING.md)); prior **3.5.0** canopy ambience, **3.2.0** seasonal canopy phenology; **3.1.12** mycelium + tree spread.  
**Версия мода:** `3.6.0` · **Игра:** Vintage Story 1.22+ · **Сборка:** .NET 10 · **Тесты:** xUnit (**257**)  

**ModDB:** https://mods.vintagestory.at/ecosystemflora  

Последнее обновление документации: 2026-06-14 (код **3.6.0**, phased senescence, handbook en/ru, [`CHANGELOG.md`](CHANGELOG.md)).

См. также: [PROJECT_VISION.md](PROJECT_VISION.md) (теория), [PROMPT.md](PROMPT.md) (промпт для агентов), [CHANGELOG.md](CHANGELOG.md) (релизные заметки), [THIRD_PARTY_ECOLOGY.md](THIRD_PARTY_ECOLOGY.md) (сторонние блоки), [GAPS.md](GAPS.md) (пробелы идеи), [CANOPY_PHENOLOGY.md](CANOPY_PHENOLOGY.md) (сезонная листва), [CANOPY_AMBIENCE.md](CANOPY_AMBIENCE.md) (частицы кроны), [TREE_AGING.md](TREE_AGING.md) (зрелость диких деревьев).
...
| **Деревья** | `log-grown-{wood}-*` → spread `sapling-{wood}-free` (14 пород); **v3.6** — годовое укрупнение, поэтапная senescence, persist возраста/фазы — [`TREE_AGING.md`](TREE_AGING.md) |
...
| `OnlyActivateNearPlayers` | Default **false** — все зарегистрированные растения в **загруженных** чанках; **true** = playtest/perf, только в радиусе игрока |
...
| **3.6.0** | **Wild tree maturation** — calendar age + structure growth; **phased senescence** (4 yearly stages); age + phase persist in savegame; inspect (I) on trunk; [`TREE_AGING.md`](TREE_AGING.md) |
...
| 5 | **Unit-тесты** — чистые функции (scoring, classification, season, mat spread, hand harvest, mycelium, canopy, tree growth/senescence, config merge) | [x] 257 tests |
