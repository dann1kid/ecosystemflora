...
namespace WildFarming.Ecosystem
{
    ...
    internal static class CanopyFoliageRules
    {
            ...
            FoliageCellIndex index)
        {
            ...
            if (!EcosystemConfig.Loaded.FoliageCatchUpOnChunkLoad) return false;
            FoliageCellKind kind = Classify(block);
            // Chunk-load path: branchy → leaf only. LogGrown spring scaffold is on the foliage
            // scheduler — full-column log catch-up on every chunk load hitch SSP hard.
        ...
        internal static bool NeedsSpringCatchUp(IBlockAccessor acc, BlockPos pos, string wood, FoliageCellKind kind)
        {
            ...
            {
                return CountFoliageInRadius(acc, pos, wood, FoliageCellKind.RegularLeaf, 2, profile.MaxRegularNearBranchy)
                    < profile.MaxRegularNearBranchy;
            }
            return false;
        }

        static readonly BlockPos FoliageCountScratch = new BlockPos(0);

        /// <summary>Counts matching foliage; stops once <paramref name="stopAt"/> is reached.</summary>
            ...
            for (int dx = -radius; dx <= radius; dx++)
            {
                for (int dy = dyStart; dy <= radius; dy++)
                {
                    for (int dz = -radius; dz <= radius; dz++)
                    {
                        ...
                        FoliageCountScratch.Set(center.X + dx, center.Y + dy, center.Z + dz);
                        FoliageCountScratch.dimension = center.dimension;
                        if (!acc.IsValidPos(FoliageCountScratch)) continue;
                        Block neighbor = acc.GetBlock(FoliageCountScratch);
