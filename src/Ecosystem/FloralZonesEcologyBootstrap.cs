using System.Globalization;
using Vintagestory.API.Common;
using Vintagestory.API.Datastructures;
using StjJsonObject = System.Text.Json.Nodes.JsonObject;

namespace WildFarming.Ecosystem
{
    /// <summary>
    /// Floral Zones uses worldgen patches that do not declare ecology attributes on blocks.
    /// Inject ecology attrs at asset finalize using the extracted worldgen climate table.
    /// </summary>
    internal static class FloralZonesEcologyBootstrap
    {
        public static void Apply(ICoreAPI api)
        {
            if (api == null || api.Side != EnumAppSide.Server) return;
            if (!EcosystemConfig.Loaded.EnableThirdPartyParticipants) return;

            int injected = 0;
            int mergeFailed = 0;
            foreach (Block block in api.World.Blocks)
            {
                if (block?.Code == null || block.Id == 0) continue;
                if (PlantCodeHelper.HasDeclaredEcologyParticipant(block)) continue;

                if (!TryBuildInjection(block, out JsonObject ecology)) continue;
                if (!MergeEcology(block, ecology))
                {
                    mergeFailed++;
                }

                injected++;
            }

            if (injected > 0)
            {
                api.Logger.Notification(
                    "[ecosystemflora] Floral Zones ecology attrs injected on {0} blocktypes ({1} merge fallbacks; path rules still apply)",
                    injected,
                    mergeFailed);
            }
        }

        static bool TryBuildInjection(Block block, out JsonObject ecology)
        {
            ecology = null;
            string fullCode = block?.Code?.ToString();
            if (string.IsNullOrEmpty(fullCode)) return false;

            if (!FloralZonesWorldgenPlants.TryGet(fullCode, out FloralZonesWorldgenPlants.Entry entry))
            {
                return false;
            }

            // Exclude harvested variants early.
            if (block.Code.Path != null && block.Code.Path.Contains("-harvested-")) return false;

            AssetLocation spread = block.Code;
            string species = ResolveSpecies(block);
            if (string.IsNullOrEmpty(species)) return false;

            string json = string.Format(
                CultureInfo.InvariantCulture,
                @"{{
  ""ecologyParticipant"": true,
  ""ecologySpecies"": ""{0}"",
  ""ecologyHabitat"": ""{1}"",
  ""ecologySpreadBlock"": ""{2}:{3}"",
  ""ecologyMeadowHarvest"": ""none"",
  ""minTemp"": {4},
  ""maxTemp"": {5},
  ""minRain"": {6},
  ""maxRain"": {7},
  ""minForest"": {8},
  ""maxForest"": {9},
  ""ecologySpreadRate"": {10},
  ""ecologySameSpeciesSpacing"": {11},
  ""ecologyOtherSpeciesSpacing"": {12}
}}",
                species,
                entry.Habitat,
                spread.Domain,
                spread.Path,
                entry.MinTemp,
                entry.MaxTemp,
                entry.MinRain,
                entry.MaxRain,
                entry.MinForest,
                entry.MaxForest,
                entry.SpreadRate,
                entry.Habitat == EcologyHabitat.TerrestrialTree ? 9 : 2,
                entry.Habitat == EcologyHabitat.TerrestrialTree ? 6 : 3);

            ecology = JsonObject.FromJson(json);
            return ecology != null;
        }

        static string ResolveSpecies(Block block)
        {
            if (block?.Code == null) return null;

            // Reuse existing parsing rules (works for flower-* across any domain).
            string viaHelper = PlantCodeHelper.GetEcologySpecies(block.Code);
            if (!string.IsNullOrEmpty(viaHelper)) return viaHelper;

            string path = block.Code.Path ?? "";

            // Common Floral Zones patterns (berry bushes, ferns, tallplants, waterlilies).
            if (path.StartsWith("smallberrybush-"))
            {
                return TakeBetween(path, "smallberrybush-");
            }

            if (path.StartsWith("bigberrybush-"))
            {
                return TakeBetween(path, "bigberrybush-");
            }

            if (path.StartsWith("tallplant-"))
            {
                return TakeBetween(path, "tallplant-");
            }

            if (path.StartsWith("fern-"))
            {
                return path.Substring("fern-".Length);
            }

            if (path.StartsWith("waterlily-"))
            {
                return path.Substring("waterlily-".Length);
            }

            // Fallback: first token after prefix up to next '-'.
            int dash = path.IndexOf('-');
            if (dash > 0 && dash + 1 < path.Length)
            {
                string rest = path.Substring(dash + 1);
                int dash2 = rest.IndexOf('-');
                return dash2 > 0 ? rest.Substring(0, dash2) : rest;
            }

            return null;
        }

        static string TakeBetween(string path, string prefix)
        {
            string rest = path.Substring(prefix.Length);
            int dash = rest.IndexOf('-');
            return dash > 0 ? rest.Substring(0, dash) : rest;
        }

        static bool MergeEcology(Block block, JsonObject ecology)
        {
            if (block.Attributes == null)
            {
                block.Attributes = ecology;
                return true;
            }

            string merged = TryMergeJson(block.Attributes.ToString(), ecology.ToString());
            if (merged == null) return false;

            block.Attributes = JsonObject.FromJson(merged);
            return true;
        }

        static string TryMergeJson(string baseJson, string overlayJson)
        {
            if (string.IsNullOrWhiteSpace(baseJson)) return overlayJson;
            if (string.IsNullOrWhiteSpace(overlayJson)) return baseJson;

            try
            {
                if (System.Text.Json.Nodes.JsonNode.Parse(baseJson) is not StjJsonObject baseObj
                    || System.Text.Json.Nodes.JsonNode.Parse(overlayJson) is not StjJsonObject overlayObj)
                {
                    return null;
                }

                foreach (var pair in overlayObj)
                {
                    baseObj[pair.Key] = pair.Value?.DeepClone();
                }

                return baseObj.ToJsonString();
            }
            catch
            {
                return null;
            }
        }
    }
}

